...
| State signature | `Get : int`, `Set : int -> unit` | `state<s>` with `fun get : () -> s`, `fun set : s -> ()` | `StateF s next = Get (s -> next) \| Set s next` |
...
| State write operation | `perform (Set n)` | `set(x)` | `set :: s -> Free (StateF s) ()` |
